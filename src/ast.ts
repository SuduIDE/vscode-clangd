...
class ASTFeature implements vscodelc.StaticFeature {
  constructor(private context: ClangdContext) {
    ...
    const tree =
        vscode.window.createTreeView('sudu.clangd.ast', {treeDataProvider: adapter});
    context.subscriptions.push(
        ...
        adapter.onDidChangeTreeData((_) => {
          vscode.commands.executeCommand('setContext', 'sudu.clangd.ast.hasData',
                                         adapter.hasRoot());
        ...
        vscode.commands.registerCommand(
            'sudu.clangd.ast.close', () => adapter.setRoot(undefined, undefined)));
  }
  ...
  initialize(capabilities: vscodelc.ServerCapabilities,
    ...
    vscode.commands.executeCommand('setContext', 'sudu.clangd.ast.supported',
